...
const VendorForm: React.FC<VendorFormProps> = ({ vendorId, onSuccess }) => {
  ...
  return (
    <Paper sx={{ maxWidth: 1000, p: 2 }}>
      ...
      <Box sx={{ mt: 2, mb: 2, display: 'flex', width: 'max-content', gap: '1px', flexDirection: 'column' }}>
        <Avatar
          variant="rounded"
          src={vendor?.image ?? ''}
          sx={{ width: 150, height: 150 }}
        />
        <Button size='small' variant="contained" onClick={() => setImageUploadOpen(true)}>{tVendorDash("uploadMedia")}</Button>
        <ImageUploader
          open={imageUploadOpen}
          onClose={() => setImageUploadOpen(false)}
          context={ImageUploaderContext.VENDOR}
          entityId={vendorId}
          width={300}
          height={300}
          onSuccess={() => { }}
