...
- compilation: camap-ts> `npm run build:front`
	- Astuce : il est possible d'utiliser `npm run watch` dans `packages/front-core` pour recompiler automatiquement les fichiers front modifiés. 
		...
		- Attention : A des fin d'optimisation en dév, lLe fichier de config webpack en dev ne génère pas le fichier palette.ts (cf `webpack.dev.config.js`). Décommenter l.74 pour le faire revenir 
